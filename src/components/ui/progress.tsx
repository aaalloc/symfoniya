...
>(({ className, ...props }, ref) => {
  ...
  return (
    <SliderPrimitive.Root
      ...
      {...props}
    >
      <SliderPrimitive.Track className="cursor-pointer group/item relative h-2 w-full grow overflow-hidden bg-gradient-to-r to-[#00C5DF] via-[#FFC700] from-[#F2371F]">
        <SliderPrimitive.Range className="absolute h-full transition-all bg-transparent" />
      </SliderPrimitive.Track>
      <div
        className={
          "absolute top-0 right-0 bg-secondary h-full transition-all cursor-pointer"
        }
        style={{ width: `${mask}%` }}
      />
